...
class SiteControllers {
    ...
    Cart.find({ cart_username_id: req.session.loginUser._id })
      ...
      .exec(function (err, cart) {
        ...
        for (let price of cart) {
          totalprice =
            totalprice +
            parseInt(price.cart_number_product) *
              parseInt(price.cart_product_id.product_price);
        }
  ...
  async showProductBuy(req, res) {
    ...
    Cart.findOne({
      ...
      .exec(function (err, cart) {
        const totalprice =
          parseInt(cart.cart_number_product) *
          parseInt(cart.cart_product_id.product_price);
        if (err) return handleError(err);
  ...
  async addproduct(req, res) {
    const findproductid = await Cart.findOne({
      cart_product_id: req.body.product_id,
      cart_isReceived: false,
    }).exec();
    if (findproductid) {
      let updateamount = parseInt(
        findproductid.cart_number_product + parseInt(req.body.product_quanity),
      );
