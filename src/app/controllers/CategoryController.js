...
class CategoryControllers {
  createCategory(req, res) {
    res.render('category/createCategory', { layout: false });
  }

  show(req, res, next) {
    Category.find()
      .then((category) =>
        res.render('category/showallcategory', {
  ...
  async showProductOfCategory(req, res, next) {
    const findIdOfCategory = await Category.findOne({
      slug: req.params.slug,
    }).exec();
    const page = parseInt(req.query.page) || 1;
    var startIndex = (page - 1) * PAGELIMIT;
    Product.find({ product_category: findIdOfCategory._id })
      .skip(startIndex)
      .limit(PAGELIMIT)
      .sort({ createdAt: -1 })
      .exec(async function (err, product) {
        try {
          ...
          const PageNumber = await Product.find({
            product_category: findIdOfCategory._id,
          }).countDocuments();
          const countCategoryProduct = await Product.find({
            product_category: findIdOfCategory._id,
          })
            .countDocuments()
            .exec();
          ...
          for (const value of findAllCategoryName) {
            const item = {
              ...
              isActiveCategory:
                value._id.toString() === findIdOfCategory._id.toString(),
            };
