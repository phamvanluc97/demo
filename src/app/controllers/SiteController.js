...
class SiteControllers{
    homepage(req,res,next)
    {
        ...
        Product.find({}).limit(3).sort({createdAt:-1}).exec(async function (err,product) {
            try{
            ...
            Product.find({}).sort({createdAt:-1}).exec(async function (err,product) {
               try{
                if (err) return handleError(err);
                const category=await Category.find().exec();
                const cartAdminCheck=await Cart.find({cart_isBuyCheck:true,cart_isAdminCheck:false}).populate('cart_product_id cart_username_id').exec();
                const cartReceivedCheck=await Cart.find({cart_isReceived:true}).populate('cart_product_id cart_username_id').exec();
                const cartDenyCheck=await Cart.find({cart_isAdminDeny:true}).populate('cart_product_id cart_username_id').exec();
    ...
    async shop(req,res,next)
    {
        ...
        Product.find({}).skip(startIndex).limit(PAGELIMIT).sort({createdAt:-1}).exec(async function (err,product) {
        try {
