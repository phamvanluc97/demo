...
class MessageControllers{
    showmessage(req,res)
    {
        let allchat=[];
        let checkchat=[];
        Message.find({message_user_sender:req.session.loginUser._id}).sort({createdAt:1}).exec(async function(err,msg)
        {
            if (err) return handleError(err);
            const adminSend=await Message.find({message_user_receiver:req.session.loginUser._id}).sort({createdAt:1}).exec();
            allchat=msg.concat(adminSend);
            for (const value of allchat) {
                    const item={
                        message:value.message,
                        check:value.message_user_sender.toString()===req.session.loginUser._id.toString(),
