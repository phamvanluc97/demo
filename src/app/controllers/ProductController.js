...
class ProductControllers {
  ...
  shop(req, res, next) {
    Product.find()
      .then((product) =>
        res.render('product/shopproduct', {
          product: multipleMongooseToObject(product),
        }),
      )
      .catch(next);
  ...
  showDetailProduct(req, res, next) {
    Product.findOne({ slug: req.params.slug }).exec(async function (
      err,
      product,
    ) {
      try {
        ...
        const produccommnent = await Comment.find({
          comment_product_id: findproduct._id,
        })
          .limit(3)
          .sort({ createdAt: -1 })
          .populate('comment_username_id')
          .exec();
